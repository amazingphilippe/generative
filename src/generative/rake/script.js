...
function generate() {
  ...
  for (var i = -40; i <= 40; i+=1) {
    let polyline = brush.map((segment) => ([segment.pos.x + segment.dir.x * i + random(-5, 1), segment.pos.y + segment.dir.y * i + random(-1, 5)]))
    svg.polyline(polyline).fill("none").stroke({color: random(palette), width: 1}).css({ "mix-blend-mode": "multiply" });
  }
}
generate();
